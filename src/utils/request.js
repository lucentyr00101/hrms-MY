...
const errorHandler = (error) => {
  if (error.response) {
    ...
    /*
      ...
      User has no role or pemission - 1013002
      Invalid token - 1011008
    */
    const tokenIssues = [1013002, 1011008]
    if (error.response.status === 400 && tokenIssues.includes(error.response.data.code) && refreshToken) {
      // notification.error({
      ...
      // TODO - wait for backend response code
...
request.interceptors.request.use(async (config) => {
  ...
  if (!token && refreshToken && !noRefresh.some((apiName) => config.url.includes(apiName))) {
    ...
    if (config.url !== '/auth/refreshToken') config.headers.Authorization = `Bearer ${token}`
  }
  return config
}, errorHandler)
