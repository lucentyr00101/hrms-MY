<template>
  <div>
    <a-modal
      :title="`${$t('table.dialog.view-payroll-summary')} - ${ moment(model?.data?.fromDate).format('YYYY') } ${ moment(model?.data?.fromDate).format('MMMM') } ${ model?.data?.sequence }`"
      :width="1400"
      :visible="visible"
    ...
    <div class="company-contri">
      <table>
        ...
        <tr>
          ...
          <td>{{ model?.data?.companyName || '--' }}</td>
          <td>{{ $t('table.dialog.total-tax-payment') }} :</td>
          <td>{{ model?.data?.totalEmployeeContribution?.totalTax || '0' }}</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        ...
        <tr>
          ...
          <td>{{ model?.data?.employeeCount || '0' }}</td>
          <td>{{ $t('table.dialog.total-epf-payment') }} :</td>
          <td>{{ model?.data?.totalEmployeeContribution?.totalEpf || '0' }}</td>
          <td><span>{{ $t('table.dialog.total-epf-payment') }} :</span></td>
          <td>{{ model?.data?.totalEmployerContribution?.totalEpf || '0' }}</td>
          <td><span>{{ $t('table.dialog.total-epf') }} :</span></td>
          <td>{{ model?.data?.totalContribution?.totalEpf || '0' }}</td>
        </tr>
        <tr>
          <td><span>{{ $t('table.dialog.total-employee-net-pay') }} :</span></td>
          <td>{{ model?.data?.totalEmployeeNetPay || '0' }}</td>
          <td>{{ $t('table.dialog.total-socso-payment') }} :</td>
          <td>{{ model?.data?.totalEmployeeContribution?.totalSocso || '0' }}</td>
          <td><span>{{ $t('table.dialog.total-socso-payment') }} :</span></td>
          <td>{{ model?.data?.totalEmployerContribution?.totalSocso || '0' }}</td>
          <td><span>{{ $t('table.dialog.total-sosco') }} :</span></td>
          <td>{{ model?.data?.totalContribution?.totalSocso|| '0' }}</td>
        </tr>
        <tr>
          <td><span>{{ $t('table.dialog.total-employer-net-pay') }} :</span></td>
          <td>{{ model?.data?.totalEmployerNetPay || '0' }}</td>
          <td>{{ $t('table.dialog.total-eis-payment') }} :</td>
          <td>{{ model?.data?.totalEmployeeContribution?.totalEis || '0' }}</td>
          <td><span>{{ $t('table.dialog.total-eis-payment') }} :</span></td>
          <td>{{ model?.data?.totalEmployerContribution?.totalEis || '0' }}</td>
          <td><span>{{ $t('table.dialog.total-eis') }} :</span></td>
          <td>{{ model?.data?.totalContribution?.totalEis || '0' }}</td>
        </tr>
      </table>
    </div>
    <div class="employee-contri">
      <table>
        ...
        <tr v-for="(value, i) in model?.data?.employeePayrollSummaries" :key="i">
