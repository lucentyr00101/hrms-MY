<template>
  <div class="main">
    ...
    >
      ...
      <!-- Google Login -->
      <!-- <div id="google-login-wrapper"></div>
...
export default {
  ...
  methods: {
    ...
    initiate() {
      // this.googleLogin() // Initiate Google Login
    },
    googleLogin() {
      const handleCredentialResponse = (response) => {
        // console.log('Encoded JWT ID token:', response.credential)
        this.Login({ type: 'google', userInfo: { accessToken: response.credential } }).then((res) => this.loginSuccess(res)).catch(err => this.requestFailed(err))
      }
      window.onload = () => {
        window.google.accounts.id.initialize({
          client_id: '506983327364-42k4rbd4h31vhb17ui90j4ieudksibch.apps.googleusercontent.com', // Google Login Client ID
          callback: handleCredentialResponse
        })
        window.google.accounts.id.renderButton(
          document.getElementById('google-login-wrapper'),
          { theme: 'outline', size: 'large', text: 'continue_with', width: '368' } // customization attributes
        )
    ...
    requestFailed (err) {
      ...
      // console.log('blabla', err.response.data.message)
