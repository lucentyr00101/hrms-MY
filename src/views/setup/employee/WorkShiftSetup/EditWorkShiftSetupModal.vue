<template>
  <div>
    <a-modal
      :title="$t('table.dialog.edit-work-shift-type')"
      :width="800"
  ...
  methods: {
    ...
    showNotification() {
      this.$notification.open({
        message: i18nRender('notification.work-shift-setup.updated'),
        ...
        async onOk() {
          try {
            ...
            const data = {
              ...
              startTime: moment(_this.form.getFieldValue('startTime')).format('HH:mm:ss'),
              endTime: moment(_this.form.getFieldValue('endTime')).format('HH:mm:ss'),
            ...
            try {
              await _this.$store.dispatch('api/setup/employee/workShiftSetup/updateWorkShiftSetup', data)
