...
import AddAllowance from '@/components/Setup/Employee/EmployeeSetup/Allowance/AddAllowance'
...
export default {
  name: 'Allowance',
  components: { ChosenAlertBox, AddAllowance, FillEmployee },
  ...
  computed: {
    ...
    columns () {
      return [
        { title: i18nRender('table.column.allowance-type'), dataIndex: 'allowanceType' },
  ...
  async created () {
    if (this.$route.params.id) {
      ...
      this.getEmployeeSetup(this.$route.params.id).then(res => {
        this.data = res.data.employeeSetupAllowanceList
      })
    }
  },
  methods: {
    ...
    cancel () {
      if (this.action === 'edit' && !this.disabled) {
        ...
        this.data = this.employee.data.employeeSetupAllowanceList
      } else {
    ...
    showNotification () {
      this.$notification.open({
        message: this.$t('table.dialog.allowance-saved'),
    ...
    handleDelete (selectedRowKeys) {
      ...
      if (selectedRowKeys.length > 0) {
        this.$confirm({
          ...
          onOk: () => {
            const newArray = _this.data.filter(item => !selectedRowKeys.includes(item.allowanceId))
    ...
    handleSubmit () {
      try {
        ...
        const data = {
          ...
          employeeSetupAllowanceList: this.data
        }
        this.$store.dispatch('api/setup/employee/employeeSetup/editEmployeeAllowance', data)
        .then((res) => {
          this.$store.commit('api/setup/employee/employeeSetup/SET_EMPLOYEE', res)
          this.$notification.open({
            message: i18nRender('table.dialog.allowance-saved'),
            icon: <a-icon type="check-circle" theme="twoTone" two-tone-color="#52c41a" />
          })
          this.action = 'edit'
          this.disabled = true
