<template>
  <a-modal
    ...
    :visible="value"
    :destroyOnClose="true"
    :title="action2 === 'edit' ? $t('table.dialog.edit-employee-allowance') : $t('table.dialog.add-employee-allowance')"
      ...
      <a-form-item :label="$t('table.column.allowance-type')">
        <a-select
          v-decorator="[
            ...
            {
              rules: [
                { required: true, message: $t('error.allowance-type.required') }
              ],
              initialValue: action2 === 'edit' ? currentValue?.allowanceType : ''
            }
          ]"
        >
        <a-select-option v-for="(allowanceTypes, index) in allowanceTypeDropdown" :key="index" :value="allowanceTypes.id" @click="e => handleSelectChange(e, allowanceTypes)">{{ allowanceTypes.allowanceType }}</a-select-option>
        </a-select>
      </a-form-item>
              ...
              <a-form-item style="width: 150%">
                <a-date-picker
                  ...
                  v-decorator="[
                    ...
                    {
                      rules: [
                        { required: true, message: 'From Date is required.' }, {validator: validateDate}
                      ],
              ...
              <a-form-item style="width: 150%">
                <a-date-picker
                  ...
                  v-decorator="[
                    ...
                    {
                      rules: [
                        { required: true, message: 'To Date is required.' }, {validator: validateDate}
                      ],
...
export default {
  name: 'EmployeeSetupAddAllowance',
  props: {
    value: Boolean,
    actionModal: {
      type: String,
  ...
  data () {
    return {
      ...
      action: null,
      loadAllowanceType: async () => {
        const data = await this.$store.dispatch('api/dropdown/fetchAllowanceDropdown')
        this.allowanceTypeDropdown = data.data.allowanceType
      },
      loadSequenceType: async () => {
        ...
        data.data.sequenceType.forEach((item) => {
          this.sequenceTypeDropdown = [...this.sequenceTypeDropdown, {
  ...
  watch: {
    'currentValue.allowanceType' (val) {
      this.allowanceType = val
    }
  },
  methods: {
    closeModal () {
    ...
    handleSubmit () {
      if (this.action2 === 'add') {
        const data = {
          allowanceId: Math.floor(Math.random() * 1000),
          allowanceType: this.allowanceType ? this.allowanceType : this.currentValue.allowanceType,
        ...
        this.$notification.open({
          message: this.$t('table.dialog.allowance-added'),
          icon: <a-icon type="check-circle" theme="twoTone" two-tone-color="#52c41a" />
        })
      } else {
        const newArray = this.$parent.data.map(obj => {
          if (obj.allowanceId === this.currentValue.allowanceId) {
            return { ...obj,
              allowanceType: this.allowanceType ? this.allowanceType : this.currentValue.allowanceType,
              sequence: this.form.getFieldValue('sequence'),
              amount: this.form.getFieldValue('amount'),
        ...
        this.$notification.open({
          message: this.$t('table.dialog.allowance-updated'),
          icon: <a-icon type="check-circle" theme="twoTone" two-tone-color="#52c41a" />
        })
      }
      this.closeModal()
